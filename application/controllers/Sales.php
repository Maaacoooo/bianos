...
class Sales extends CI_Controller {
		...
		if($userdata)	{
			...
			if($data['user']['usertype'] == 'Administrator') {
				...
				$this->load->view('sales/create', $data);
					...
					//Proceed saving candidate				
					if($this->sales_model->create()) {		
					
						$this->session->set_flashdata('success', 'Succes! New Sales created!');
						redirect($_SERVER['HTTP_REFERER'], 'refresh');
					} else {
						//failure
						$this->session->set_flashdata('error', 'Oops! Error occured!');
	...
	public function autocomplete(){
	    ...
	    if (isset($_GET['term'])){
	      ...
	      $result = $this->sales_model->search_menu($q);

	      foreach($result as $row) {
	      	$new_row['label']=htmlentities(stripslashes($row['title'] . ' - ' . $row['price']));
            $new_row['value']=htmlentities(stripslashes('#'. prettyID($row['id']) . ' -- ' . $row['title'] . ' - ' . $row['price']));
